...
def format_amazon_kw(elements, global_kws):
    """SEO 權重優化：融合 AI 提取的多元素與全局詞庫"""
    # 結合 AI 提取的多個特徵元素與通用關鍵詞
...
# --- 2. 頁面配置 ---
st.set_page_config(page_title="亞馬遜專家 V65", layout="wide")
api_key = os.getenv("OPENAI_API_KEY") or st.secrets.get("OPENAI_API_KEY") or ""

st.title("🔥 亞馬遜 AI 批量上架系統 V65")
st.success("✅ SEO 深度優化：多元素特徵抓取 + 流量詞深度融合 + 變體差異化描述。")
...
with st.sidebar:
    ...
    global_kws = st.text_area("✨ 全局關鍵詞詞組庫", "girls summer dress bunny pattern floral print casual wear")
    st.divider()
    size_matrix = []
...
# --- 4. 款式管理 ---
if 'v65_rows' not in st.session_state: st.session_state.v65_rows = 1
sku_items = []
for i in range(st.session_state.v65_rows):
...
if st.button("➕ 增加一個款式"):
    st.session_state.v65_rows += 1; st.rerun()

tpl_file = st.file_uploader("📂 上傳 Amazon 模板", type=['xlsx', 'xlsm'])

# --- 5. 核心執行 ---
if st.button("🚀 啟動 V65 SEO 強化生成", type="primary") and tpl_file and api_key:
    with st.spinner('⏳ AI 正在地毯式掃描圖像特徵並融合關鍵詞...'):
        log_area = st.empty()
        ...
        try:
            ...
            # --- 核心 AI 策略升級：多維度特徵提取 ---
            prompt_rules = """Analyze image deeply for Amazon SEO. 
            Output JSON: { 
                "detailed_element": "Comprehensive pattern description. Include subject + tiny details + background + style (e.g. 'Bunny with Pink Floral Garland and Garden Basket')",
                "short_element": "Concise core element for Color Map (e.g. 'Floral Bunny')",
                "common_desc": "SEO rich category phrase 100-150 chars, utilizing provided keyword context.",
                "bp": ["Bullet point with product-specific details (25+ words).", "Bullet point...", "Bullet point...", "Bullet point...", "Bullet point..."],
                "desc": "HTML desc" 
            }
            Strategy: Don't just say 'Bunny'. Scan for flowers, baskets, butterflies, colors, and textures. Ensure variations are highly distinct."""
            ...
            for col_idx, val in fixed_values.items():
                if not sheet.cell(row=row_cursor, column=col_idx).value: sheet.cell(row=row_cursor, column=col_idx, value=val)
            row_cursor += 1

            # B: 子類
            for step, item in enumerate(valid_items):
                log_area.text(f"⏳ 正在地毯式提取款式 #{step+1} 的流量元素...")
                item["img"].seek(0)
                ...
                base_child_title = deduplicate_title(f"{brand} {det_el} {fixed_desc}")
            ...
            log_area.text("✅ V65 深度 SEO 填充完成！")
            st.download_button("💾 下載深度優化版文件", out.getvalue(), "Amazon_V65_SEO.xlsm")
